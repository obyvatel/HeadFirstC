...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		...
		if( !fork() )   // if child process created fork=0
		{
		    close(listener_d);  // close main listening socket
		    ...
		    close(connect_d);   // after message exchanging close secondary socket
		    exit(0);            // exit app
		}
		
        close(connect_d);       // if child process do not created
	}
	return 0;	
}
